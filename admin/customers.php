  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'deleteconfirm':
        ...
        break;
        case 'editpointsconfirm':
        $customers_id = tep_db_prepare_input($HTTP_GET_VARS['cID']);
        $customers_reward_points = tep_db_prepare_input($HTTP_POST_VARS['customers_reward_points']);
        if($customers_reward_points != '') {
        tep_db_query("update " . TABLE_CUSTOMERS . " set customers_reward_points = '" . tep_db_input($customers_reward_points) . "' where customers_id = '" . (int)$customers_id . "'");    
        }
        tep_redirect(tep_href_link('customers.php', tep_get_all_get_params(array('action'))));
        break;
      default:
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, c.customers_reward_points, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
...
<?php
    ...
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, c.customers_telephone, c.customers_reward_points, a.entry_country_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . $search . " order by c.customers_lastname, c.customers_firstname";
  ...
  switch ($action) {
    case 'confirm':
      ...
      break;
       ...
       case 'editpoints':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_EDIT_POINTS . '</strong>');
      $contents = array('form' => tep_draw_form('customers', 'customers.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=editpointsconfirm'));
      $contents[] = array('text' => '<br />' . TEXT_EDIT_POINTS . ' ' . tep_draw_input_field('customers_reward_points', $cInfo->customers_reward_points, 'size="4"'));
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id)));
      break;
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        $heading[] = array('text' => '<strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');

	$contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('customers.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('customers.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm')) . tep_draw_button(IMAGE_POINTS, 'transfer-e-w', tep_href_link('customers.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=editpoints')) . tep_draw_button(IMAGE_ORDERS, 'cart', tep_href_link('orders.php', 'cID=' . $cInfo->customers_id)) . tep_draw_button(IMAGE_EMAIL, 'mail-closed', tep_href_link(mail.php, 'customer=' . $cInfo->customers_email_address)));
...
$contents[] = array('text' => '<br />' . TEXT_DATE_ACCOUNT_CREATED . ' ' . tep_date_short($cInfo->date_account_created));
        ...
        $contents[] = array('text' => '<br />' . TEXT_INFO_REWARD_POINTS . ' ' . $cInfo->customers_reward_points);
