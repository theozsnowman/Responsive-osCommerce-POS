...
*/
  ...
  if (isset($HTTP_GET_VARS['filename'])) {
    $file_edit = realpath(DIR_FS_ADMIN_LANGUAGES . $HTTP_GET_VARS['filename']);

    if (substr($file_edit, 0, strlen(DIR_FS_ADMIN_LANGUAGES)) != DIR_FS_ADMIN_LANGUAGES) {
      tep_redirect(tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir']));
    }
  }
    ...
    switch ($action) {
      case 'save':
        if (isset($HTTP_GET_VARS['lngdir']) && isset($HTTP_GET_VARS['filename'])) {
          $file = DIR_FS_ADMIN_LANGUAGES . $HTTP_GET_VARS['filename'];
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><?php echo tep_draw_form('lng', 'define_language_admin.php', '', 'get'); ?>
...
?>
          <tr><?php echo tep_draw_form('language', 'define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $HTTP_GET_VARS['filename'] . '&action=save'); ?>
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
              ...
              <tr>
                <td class="smallText" align="right"><?php if ($file_writeable == true) { echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'])); } else { echo tep_draw_button(IMAGE_BACK, 'arrow-1-w', tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'])); } ?></td>
              </tr>
            </table></td>
          </form></tr>
          <tr>
...
?>
          ...
          <tr>
            <td><?php echo tep_draw_button(IMAGE_BACK, 'arrow-1-w', tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'])); ?></td>
          </tr>
<?php
    }
  } else {
          ...
          <tr>
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">
                <td class="dataTableContent"><a href="<?php echo tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $filename); ?>"><strong><?php echo $filename; ?></strong></a></td>
                <td class="dataTableContent" align="center"><?php echo tep_image(DIR_WS_IMAGES . 'icons/' . ((tep_is_writable(DIR_FS_ADMIN_LANGUAGES . $filename) == true) ? 'tick.gif' : 'cross.gif')); ?></td>
                <td class="dataTableContent" align="right"><?php echo strftime(DATE_TIME_FORMAT, filemtime(DIR_FS_ADMIN_LANGUAGES . $filename)); ?></td>
              </tr>
<?php
    foreach (tep_opendir(DIR_FS_ADMIN_LANGUAGES . $HTTP_GET_VARS['lngdir']) as $file) {
      if (substr($file['name'], strrpos($file['name'], '.')) == $file_extension) {
        $filename = substr($file['name'], strlen(DIR_FS_ADMIN_LANGUAGES));

        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' .
             '                <td class="dataTableContent"><a href="' . tep_href_link('define_language_admin.php', 'lngdir=' . $HTTP_GET_VARS['lngdir'] . '&filename=' . $filename) . '">' . substr($filename, strlen($HTTP_GET_VARS['lngdir'] . '/')) . '</a></td>' .
