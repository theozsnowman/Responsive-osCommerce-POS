...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($_GET['action']) {
      ...
      case 'buy_direct' :    if (isset($HTTP_GET_VARS['products_id']) && is_numeric($HTTP_GET_VARS['products_id'])) {
        if (isset($HTTP_GET_VARS['id'])) {
        $pre_attributes = explode('-', $HTTP_GET_VARS['id'], 2);
        
        $attributes = array($pre_attributes[0] => $pre_attributes[1]);    
        } else {
        $attributes = '';    
        }
                                $cart->add_cart($HTTP_GET_VARS['products_id'], $cart->get_quantity(tep_get_uprid($HTTP_GET_VARS['products_id'], $attributes))+1, $attributes);
                              }
